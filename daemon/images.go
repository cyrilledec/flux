...
func (d *Daemon) pollForNewImages(logger log.Logger) {
	...
	changes := calculateChanges(logger, workloads, imageRepos)
...
func calculateChanges(logger log.Logger, workloads []cluster.Workload, imageRepos update.ImageRepos) *update.Automated {
	...
	containers:
		for _, container := range workload.ContainersOrNil() {
			...
			pattern := policy.GetTagPattern(workload.Policies, container.Name)
